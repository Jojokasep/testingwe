...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!amount || amount < 1000) return NextResponse.json({ success: false, message: "Minimal transfer Rp 1.000" });
        const result = await hitAPI('https://app.orderkuota.com/api/v2/get', { 'requests[qris_withdraw][amount]': amount });
        if (result.success && result.qris_withdraw?.success) return NextResponse.json({ success: true, message: `Saldo Rp ${Number(amount).toLocaleString('id-ID')} berhasil dipindahkan ke saldo utama.` });
        return NextResponse.json({ success: false, message: result.message || "Gagal memproses transfer" });
    } catch (e) { return NextResponse.json({ success: false, message: "Terjadi kesalahan server" }); }
}